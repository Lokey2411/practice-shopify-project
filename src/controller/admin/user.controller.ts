...
export const getAllUsers = async (req: Request, res: Response) => {
    try {
        const filter = applyFilter(req.query, ['username', 'email', 'permission'])
        const sort = applySort(req.query, ['username', 'createdAt'])
        const users = await User.find(filter).sort(sort)
        return res.json({ message: 'Lấy danh sách người dùng thành công', data: users })
    } catch (error) {
        return res.status(STATUS.INTERNAL_SERVER_ERROR).json(`Lỗi khi lấy danh sách người dùng: ${(error as Error).message}`)
    }
}
